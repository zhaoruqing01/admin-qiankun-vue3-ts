# 微应用 1 (Son01) - Qiankun 微前端子应用

这是一个基于 **Qiankun 微前端框架**的 Vue 3 + TypeScript + Vite 微应用。该应用被主应用动态加载和卸载，支持路由隔离和全局状态管理。

## 项目特性

- 🚀 **Vue 3 + TypeScript + Vite** - 现代化的前端技术栈
- 🎯 **Qiankun 微应用** - 由主应用动态加载的独立应用
- 🔄 **全局状态通信** - 通过 props 与主应用通信
...
应用将在 `http://localhost:8081` 启动。可以独立测试本应用的功能。
...
4. 点击导航进入 `/son01-vue3-ts` 路由，主应用将加载此微应用
...
- `dist/son01-vue3-ts.umd.js` - UMD 格式（兼容各种导入方式）
- `dist/son01-vue3-ts.es.js` - ES 模块格式
...
plugins: [
  vue(),
  qiankun('son01-vue3-ts', { useDevMode: true }),
],
...
build: {
  lib: {
    ...
    name: 'son01-vue3-ts',
    formats: ['umd', 'es'],
    fileName: (format) => `son01-vue3-ts.${format}.js`,
  },
  rollupOptions: {
    // 声明外部依赖（不打包进微应用）
...
export async function bootstrap() {
  console.log('son01-vue3-ts bootstrap');
}

export async function mount(props: any) {
  console.log('son01-vue3-ts mount', props);
  app.mount('#app');
}

export async function unmount() {
  console.log('son01-vue3-ts unmount');
  app.unmount();
}
```
...
2. **端口配置** - 确保微应用端口与其他应用不冲突（本应用使用 8081）
3. **容器元素** - 确保根组件挂载到 `#app` 元素
4. **依赖共享** - Vue 和 Vue Router 声明为外部依赖，由主应用统一提供
